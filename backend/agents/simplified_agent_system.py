...
class BaseAgent(ABC):
        ...
        for line in lines:
            ...
            if re.match(r'^\d+\.|\-|\*|•', line):
                ...
                current_rec = {
                    ...
                    'details': self._extract_specific_details_from_response(response, title, pillar)
                }
    ...
    def _extract_specific_details_from_response(self, response: str, title: str, pillar: str) -> str:
        """Extract specific details from LLM response that relate to the given recommendation title"""
        
        # Split response into sections and paragraphs
        ...
        # Look for sentences that contain multiple title keywords
        relevant_sentences = []
        
        for sentence in sentences:
            sentence_lower = sentence.lower()
            matches = sum(1 for word in title_words if word in sentence_lower)
            
            # If sentence contains 2+ keywords from title, it's likely relevant
            if matches >= 2 and len(sentence) > 30:
                relevant_sentences.append(sentence)
            # Or if it contains 1 keyword and has recommendation indicators
            elif matches >= 1 and any(indicator in sentence_lower for indicator in ['because', 'should', 'will', 'can', 'helps', 'provides', 'enables', 'reduces', 'improves']):
                relevant_sentences.append(sentence)
        
        # If we found relevant sentences, combine the best ones
        if relevant_sentences:
            # Take first 1-2 most relevant sentences
            details = '. '.join(relevant_sentences[:2])
            
            # Limit length and clean up
            if len(details) > 250:
                details = details[:247] + "..."
            
            return f"🤖 LLM Analysis: {details}"
        
        # Fallback: look for any sentences that mention key concepts from the title
        fallback_sentences = []
        for sentence in sentences:
            sentence_lower = sentence.lower()
            
            # Check for any title keyword
            if any(word in sentence_lower for word in title_words) and len(sentence) > 20:
                fallback_sentences.append(sentence)
                break
        
        if fallback_sentences:
            details = fallback_sentences[0][:200]
            return f"🤖 AI Insight: {details}"
        
        # Last resort: look for sentences with action words related to the pillar
        pillar_actions = {
            'reliability': ['backup', 'recovery', 'availability', 'redundancy', 'failover', 'monitoring'],
            'security': ['encrypt', 'secure', 'protect', 'authentication', 'authorization', 'firewall'],
            'cost optimization': ['cost', 'save', 'optimize', 'reduce', 'efficient', 'budget'],
            'operational excellence': ['monitor', 'automate', 'deploy', 'manage', 'operate', 'process'],
            'performance efficiency': ['scale', 'performance', 'fast', 'optimize', 'cache', 'speed']
        }
        
        pillar_key = pillar.lower().replace(' ', '').replace('-', '')
        action_words = pillar_actions.get(pillar_key, ['improve', 'enhance', 'optimize'])
        
        for sentence in sentences:
            sentence_lower = sentence.lower()
            if any(action in sentence_lower for action in action_words) and len(sentence) > 30:
                details = sentence[:180]
                return f"🤖 {pillar} Focus: {details}"
        
        # Absolute fallback with actual pillar-specific content
        return f"🤖 AI Analysis: This {pillar.lower()} recommendation addresses key architectural patterns identified in your system. Implementation will enhance {pillar.lower()} posture through targeted improvements."
...
class WellArchitectedOrchestrator:
    ...
    def _synthesize_results(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Calculate overall scores and create pillar scores array for frontend compatibility
        total_score = 0
        pillar_count = 0
        pillar_scores = []
        ...
        for pillar_name, result in valid_results.items():
            if "analysis" in result and "overall_score" in result["analysis"]:
                ...
                # Create frontend-compatible pillar score object
                pillar_score = {
                    "pillar_name": pillar_name,
                    "overall_score": score,
                    "max_score": 100,
                    "percentage": score,
                    "sub_categories": result["analysis"].get("sub_categories", {})
                }
        ...
        return {
            ...
            "pillar_scores": pillar_scores,  # Now returns array of objects for frontend compatibility
